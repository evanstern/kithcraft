...
- **TASK-0021 / I1 (`sonnet` · `cc/claude-sonnet-5[1m]` — default tier: config plumbing
  ...
  TASK-0014, TASK-0019.
- **TASK-0023 / M8 (`sonnet` · `cc/claude-sonnet-5[1m]` — default tier: assembly of
  merged, tested parts into Runtime; the deferred judgment calls are settled by the
  written surfaces the card cites — LANE AMENDMENT 2026-08-31, operator wire-first
  ruling at checkpoint 5)** — wire E2/E3 deliberation and E4/E5 conversation into
  cmd/minddaemon: triggers, §5.5 interrupt on live ingest, dusk exchange off the pair
  signal with body-to-persona binding, ambient pool, FirstTokenLatency into the session
  report. After TASK-0021; **blocks TASK-0022's Phase 2**.
- **TASK-0022 / I2 (`sonnet` · `cc/claude-sonnet-5[1m]` — default tier: run the
  ...
  real-time with the player present) — see checkpoints. After everything incl.
  TASK-0023.
...
5. **TASK-0022 is operator-run:** the evening needs the player present ~3 hours at 1x.
   <!-- Checkpoint-5 ruling (2026-08-31, operator): staging found the daemon wires only
        E1+E6 (I1's recorded minimal-wiring scope); the operator chose WIRE-FIRST over
        run-with-gaps. TASK-0023/M8 carded and inserted into lane 5 ahead of the
        evening; TASK-0022's Phase 2 waits on its merge. -->
...
TASK-0007..0022 plus the amendment task TASK-0023 all Done on the board via one merged PR each (seventeen PRs); every card
...
| 2026-08-31 | TASK-0023 | — | — | — | in flight (lane 5 amendment, blocks TASK-0022 Phase 2; TASK-0022's own claim/staging rows ride its still-open branch): carded (27965ab, board-sync at root) per the operator's wire-first ruling; claimed (a77de94), runbook amended (9e6d5ed), spec cycle (9d5a380), phase ACs seeded + tier note (692cc9c). phases: 1 done (b4c2c8d, sonnet verified, ~357k; vendor.go promotes the evening-test envelope contract reading conn/session/seq live under rt.mu (reconnect-safe); deliberation.go — per-body goroutine + trigger queue, §2.2 classification ponytailed to self_state/notable/completed-act_result, windowSnapshot over memory.Log.Events() (already the smallest export — M5's deviation closed without new surface), personaFor placeholder body==CastID for Phase 2, singleRoundProposer ErrDone; additive seam glue: Ingester.OnSessionOpen (capabilities only arrive at session_open, loop's Vendor doc requires declared verbs); §5.5 interrupt live — cancel + no-own-call + one follow-up proven through the real listener under -race, multi-urgent coalescing deliberately left to the package-level deterministic proof (recorded); pre-existing seam test-harness race found+fixed test-only; orchestrator re-ran vet+test -race green uncached), 2 done (65cfedf, sonnet verified, ~294k; binding investigated against the real wire — manifest is world-independent so body==CastID promoted to THE mechanism, bound once at attach + opportunistic bindPersonaIfUnbound off the pairing signal's descriptor; E6 prefix gains real persona text (I1 ponytail closed), E4 reuses E2/E3's prefix; isPairSignal classifies the exact PairingSignal shape — first signal fills the opener's Slot, second IS convergence (no arrival percept exists), 30s pairConvergeTimeout with identity-checked timers; ambient trigger = non-pairing person sighting, doing-present escalates; turnLatencies per villager into report.go; one -race fix in a Phase 1 test helper recorded; orchestrator re-ran vet+test -race green uncached), 3 done (a52f10e/ffdf5dd, sonnet verified, ~250k; FR-006 proof set consolidated under TestFR006_ProofSet — the one gap (single-turn dusk test) fixed to genuinely multi-turn (2 model hits, alternating-body intents, latency both speakers, 5x -race stable); dev-server observation in rehearsal mode (no key, zero calls confirmed via all-zero report): bring-up + attach + restart persistence + heartbeat confirmed live; NEITHER live deliberation nor exchange reached the daemon — STRUCTURAL findings, not timing: (1) NEW — live body tokens are opaque (b-14), never CastIDs, so T004's binding can never fire on a real server (not glue-sized, flagged for operator); (2) pre-existing — DuskPairing's signal never rides a live session (BodySession single-body ceiling); (3) self_state admissibility still open. Orchestrator re-ran vet+test -race green uncached), 4 done (3d04c0d/2d49228, sonnet verified, ~170k; gates green -race, mod/ untouched (no gradle needed, no new Mixins), OnSessionOpen recorded as mind-side-not-wire surface; body-protocol-seam genuinely stale re-verified + re-pinned, overview addendum shrunk honestly (E2-E5 wired + daemon-proven, live binding structurally blocked) + size-budget exemption for pre-existing debt, CAPSULES regenerated, freshness 11/11; ACs #1-#6+#8 ticked with citations — #3 ticked with live-blocked caveat (machinery-level wording judged satisfied, live exercise is #7's), #7 UNTICKED honestly with the three structural findings; operator findings note on the card — binding gap is the pre-evening decision, (2)/(3) routed to refactor-triage). PR next (operator-authorized merges). ~1.07M subagent tokens across 4 sonnet dispatches. |
| 2026-08-29 | TASK-0021 | — | — | — | in flight (lane 5; deps M3/M7/V3/V5 merged): claimed (239f5f7), spec cycle (ae0004d), phase ACs seeded + tier note (62c710a). phases: 1 done (a822030, sonnet verified, ~265k; runtime.go — Runtime assembling tested parts: per-body M2 log + M7 ledger/archive under rundir, LoadOrGenesisCast with partial-genesis resume + loud no-key failure, Ingester.Archived wired (TASK-0018 deferral CLOSED), HandlePercept → Gate.Decide → log; sleep derived from world_time crossings (no wire sleep message — FR-006), CycleTicks=24000 as R-1 ruling constant, no-marker retry proven end-to-end through the daemon loop; T003 call: TASK-0020 token-namespace gap FIXED as glue — BodyTokenLookups.combine merging seat + live-attach namespaces, both call sites converted, plain-JUnit proof of the exact bug shape; scope note: lookup gap only, Activity.WORK timing still open for live re-observation in Phase 3; orchestrator re-ran go + gradle gates green uncached), 2 done (bdbd43e, sonnet verified, ~162k; env-mirrored flags (MINDDAEMON_SOCKET/RUNDIR/GENESIS, flag wins), -genesis=false rehearsal path decoupled from key presence; R-6 lever: hostile despawn within 24 blocks of cast via the existing ENTITY_LOAD hook — no new Mixin, surface stays 4, off by default, recorded-take-only doc + upgrade ponytail; R-3 verified config with citation; config-not-constant audits both sides; session-end report unconditional on shutdown (LIFO defer before Close), stdout + session-report.log, zeroed rows never omitted; found+closed Phase 1 gap — memory.Instrument existed but was never wired, now recording on every admission; 175 gradle tests + full go suite green; orchestrator re-ran gates green uncached), 3 done (2c8dc76, sonnet verified, ~234k; docs/design/demo-runbook.md — build both artifacts, either start order, rehearsal -genesis=false, R-1 as ruling, R-3/R-6 via JAVA_TOOL_OPTIONS, session-report.log location; bringup-observation.md — rehearsal both sub-cases proven zero-call, both start orders attached, RESTART INDEPENDENCE CONFIRMED via seamtest double against the live daemon (memory pre-kill, SIGTERM, restart, same token reconnect, second memory, gap visible not backfilled), report on both lifetimes; one doc gap found+fixed mid-run (mod/run pre-exists); honest open findings: mod's self_state-only heartbeat admits nothing (Gate rules), Continuity.java always firstSession() + fresh token per attach — mod-side reconnect identity gap flagged, no code touched), 4 done (fad1905/babb75a, sonnet verified, ~165k; gates green --rerun-tasks, overview + body-protocol-seam re-pinned honestly with precise not-closed addendum (live E6/first-token/converse wiring stay I2's; reconnect proof used the seamtest double), CAPSULES regenerated, freshness green; 10 ACs + DoD ticked with citations, AC #2 honestly caveated; both Phase 3 findings flagged for refactor-triage/I2 in a dedicated append-note). Orchestrator: base on current origin/main, gates + probe re-run green uncached. PR next (operator-authorized merges). ~826k subagent tokens across 4 sonnet dispatches. |
| 2026-08-28 | TASK-0020 | — | — | — | in flight (lane 4 tail, serialized behind TASK-0016 — dependency satisfied by PR #22): claimed (ff39c55), spec cycle (f069c97), phase ACs seeded + tier note (58ee050). phases: 1 done (c9e7bde, sonnet verified, ~220k; board/ package — designated lectern at fixed config offset (deviation recorded: over first-placed convention), deliberately NOT POI-registered, accepts signed or unsigned book (no form, no syntax); BoardVisit rides Activity.MEET exactly as DuskPairing, zero new Activity/Mixin (count stays 4); read percept through existing Testimony/PerceptEmitter, BoardReadProtocolTest structural zero-extension proof; claims surface modeled (recordClaim hook for Phase 2); deviation: read composed+logged like DuskPairing's precedent, multi-body session wiring ponytailed; 138 tests 0 failures; orchestrator re-ran gradle green), 2 done (5db73c6, sonnet verified, ~280k; ClaimRegistry seam + BoardClaims, IntentHandler claim case — board bookkeeping not a world act; verb via declared-extras (floor verbs don't fit take-this-posting), L-7 holds; AR-4 target = board's own thing token from read provenance, zero new target fields (BoardClaimProtocolTest); T005 honestly recorded as M5-shaped scripted intent (no fixture-export path in mind/deliberate) with verb/target/reason verbatim from board_test.go; no-force-claim via call-site cardinality + package-private tryClaim; V5 tend-grave seam genuinely closed (postNotice ← LiveDeathHandling); 151 tests 0 failures; orchestrator re-ran gradle + wire vectors green), 3 done (eab23a9, sonnet verified, ~181k; build/ package 8 classes — BlueprintParser 4 shapes x 4 materials, earliest-keyword-wins, nonblank gibberish parses to defaults (claim-vs-decline stays the mind's); creative-provision sourcing recorded over storage-withdrawal (thinnest-system constraint, upgrade ponytailed); BuildEngine.advance gated on one working boolean from Activity.WORK — dusk and PANIC interrupts intentionally identical, cursor SavedData-persisted with blueprintText snapshot (board-edited-mid-build free); single-posting model kept (recorded call); unreachable-site edge structurally sidestepped (direct world write, recorded); .gitignore build/ anchor fix; 168 tests 0 failures; no new Mixin, surface 4; orchestrator re-ran gradle green), 4 first two dispatches STOPPED (both idle-looped on log monitors despite explicit anti-wait rules — ~420k tokens, nothing committed; partial live evidence gathered: session attach + book-on-lectern proven, JOB_SITE memory absent as TASK-0014 flagged); closer finished (1810f59/6055fd0/13fe153/f3e2bdf, sonnet verified, ~315k; post/read/claim OBSERVED LIVE — book across restarts, BoardVisit read fired, claim completed + first-accepted-wins reconfirmed across restart; build placement NOT observed with precise root cause: LiveBuildExecution.findClaimant checks DuskPairing seat tokens while live claims ride BodySession's generic attach token — two disjoint namespaces that never match, documented for TASK-0022 not fixed (out of T010 scope); TokenRegistry mints fresh tokens per boot (recorded); body-protocol-seam NEEDS-REVIEW resolved (claim = first declared-extras verb), overview amended, promptworld-lineage size-budget trimmed, freshness 11/11; 9 ACs ticked incl. honest live-half-deferred-to-I2 notes on #5/#6; closer's stray root-side MCP edit surfaced honestly — orchestrator discarded it (branch authoritative). Orchestrator ticked phase-mirror ACs (38f3862), gates re-run green uncached. PR next (operator-authorized merges). ~1.6M subagent tokens across 7 dispatches incl. two stopped idle-loopers. |
| 2026-08-28 | TASK-0019 | — | — | — | in flight (lane 4, parallel): claimed (49343bf), spec cycle (536137b), phase ACs seeded + tier note with named escalation trigger (5e4bb7c). phases: 1 done (fa57d19, sonnet VERIFIED serving claude-sonnet-5, ~179k; research/death-26.2.md at the brain-26.2.md standard — R-4: NO engine-side POI cooldown, grief period must be explicit vendor hold; R-5: siege trigger is VillageSiege.tick sole entry, one HEAD-cancel injection suffices, eligibility is pure POI density at 26.2 (no door/population count) so a 3-villager cast always qualifies; verdict GO — suppression point matches death §1, Mixin surface lands at 4 = decision-0002 ceiling; escalation trigger did NOT fire), 2 done (59e124e, sonnet verified, ~148k; VillageSiegeMixin HEAD-cancel exactly as planned + ZombieConversionMixin on convertVillagerToZombieVillager (sole call site, 5-bytecode body, HEAD-cancel returns false — victim's in-flight die() falls through normally, R-6 appended to death-26.2.md with javap evidence); Mixin count 4 = decision-0002 ceiling, MixinConfigTest raised; StructuralAbsenceTest for both spell-breakers; 113 tests 0 failures; live zero-siege half explicitly deferred to Phase 4; orchestrator re-ran gradle build+test green), 3 done (f623b42, sonnet verified, ~207k; GravePlacement bounded deterministic ring search with always-place fallback; GriefPeriod explicit hold via PoiManager.take/release — R-4 addendum: releasePoi frees ticket but never erases HOME/JOB_SITE memory so GlobalPos stays readable in AFTER_DEATH; capture via Fabric API ServerLivingEntityEvents.ALLOW_DEATH before dropAllDeathLoot — NO new Mixin, surface stays 4/4; GraveBoardEntry rides V2 Testimony.textContent (V4-decoupling seam, AC #6 judged satisfied); config kithcraft.griefPeriodTicks default 24000; deviations ponytailed: empty chest placed, fixed chest offset, single-ticket hold; 123 tests 0 failures; orchestrator re-ran gradle green), 4 first dispatch: T010 done (60deeb5, percept-channel proofs), then agent STOPPED after idle-waiting twice on a passive monitor for a random zombie kill (TASK-0014 idle-loop precedent); observability logging left uncommitted. Closer finished after two orchestrator resumes (008e0c2/8b87adc/cb26901/693208e, sonnet verified, ~330k incl. stalls; forced /damage kill — same ALLOW_DEATH/AFTER_DEATH path, recorded as deliberate; observed: grave sign + belongings chest placed, grief hold started + released at exactly the configured tick (home only — Aldric had no JOB_SITE claim, pre-existing TASK-0014 gap), zero sieges across two server lifetimes, retired token b-5 absent from restart boot registry while survivors persist; not observed honestly: non-empty belongings carry, live witness delivery (T010 unit proofs cover); 127 tests 0 failures; villager-brain-api amended for V5 facts but NOT re-pinned (stale from TASK-0014 debt whose fix rides PR #21 — coordination recorded); overview re-pinned. ACs #1-#5,#7-#11 agent-ticked; AC #6 ticked by orchestrator ruling per plan's V4-decoupling section (c3cc856). PR #24 MERGED 3a7f572 (operator-authorized sweep merge; reconciled first — overview union-merged + re-pinned, villager-brain-api re-pinned covering V5 facts + main's TASK-0014 fix, probe green); card synced Done (b00e5fe, board-sync at root); worktree+branch removed after merge verified. ~1.06M subagent tokens across 5 dispatches incl. one stopped idle-looper. |
| 2026-08-28 | TASK-0018 | — | — | — | in flight (lane 4, parallel with TASK-0016/0017): claimed (ee4aaed), spec cycle (ac47cfb), phase ACs seeded + tier note (4269b7b). phases: 1 done (ef88691, sonnet VERIFIED serving claude-sonnet-5, ~223k; mind/consolidate — RunNight sleep-trigger entry on world_time only, append-only JSONL Ledger with Watermark reducer, m1..mN ordinal round-trip, no-marker-on-failure incl. truncation via StopReasonMaxTokens, empty-night-lands-marker, multi-night accumulation; llm.Digest widened additively completing M4's ponytail; no-Batch-API structural grep; deviations recorded: worldTime passed directly (no sleep percept type exists yet — seam wiring deferred), plain JSON ledger (no wire identity), E6 prompt beliefs-slice deferred; orchestrator re-ran vet+test green uncached), 2+3 done grouped (287f2e2/548d92b, sonnet verified, ~166k; deathcarry.go pure read-time weighting — spike 10.0 citing I's salience band, halves per cycle, floors 1.0 never zero (RM-7), exported SelectionWeights hook for M6 at merge, deterministic-curve tests only; archive.go append-only JSONL registry idempotent by MindID, Ingester.Archived hook consulted on first + multiplexed session_open refusing fail-closed archived_mind, death-before-own-consolidation edge proven; deviation: mind identity == body token until a resolution layer exists (ponytailed); 47 tests green in touched packages; orchestrator re-ran vet+test green uncached), 4 done (e8009aa/dd704d3, sonnet verified, ~158k; gates green, body-protocol-seam NEEDS-REVIEW amended for the archival refusal + sources extended + re-pinned, promptworld-lineage + overview amended (machinery ported for real; salience-number reuse verified mind-side only), CAPSULES regenerated; 10 ACs ticked with citations, AC #5 honest caveat — hook distribution-tested, M6 consumption at merge; daemon-wiring deferral named for refactor-triage). Orchestrator: base on current origin/main, gates + probe re-run green uncached (villager-brain-api STALE = pre-existing main debt, fix rides PR #21). PR #23 MERGED 5dd2c48 (operator-authorized sweep merge; reconciled first — overview + promptworld-lineage union-merged and re-pinned to merge result, probe green); card synced Done (b00e5fe); worktree+branch removed after merge verified. ~747k subagent tokens across 4 sonnet dispatches (one grouped 2+3). |
| 2026-08-28 | TASK-0016 | — | — | — | in flight (lane 4, first task): claimed (b676d49; TASK-0014's dropped Spec marker restored at root 253b337 during claim), spec cycle (d7d595e), phase ACs seeded + tier note (a0923b4). phases: 1 done (78eb649, sonnet VERIFIED serving claude-sonnet-5, ~206k; mind/deliberate — bounded loop onto intent/act_result, OnFact only from Deliver wired to a real memory.Gate, manifest-only verbs with source-scan guard, token-only targets; Proposer/ErrDone multi-round contract recorded as judgment call, MaxIterations=5 ponytailed; orchestrator re-ran vet+test green), 2 done (7986220, sonnet verified, ~133k; e3.go — TriggerE3 on text/origin:read, E3Context four-field §2.3 render with stable-prefix byte-identity test; claim + decline authored-reason proofs, empty-reason rejected before compose — 3-line Run check generalizing FR-004 across classes, recorded deviation, no Phase 1 test broke; politeness source-scan; orchestrator re-ran vet+test green uncached), 3 done (b28f0bb, sonnet verified, ~152k; interrupt.go mutex'd state machine — cancel + at-most-one-enqueue coalescing window, Drain reopens; no Proposer/Vendor field so structurally cannot fire a call; window.go SelectWindow pure top-K-2 half-per-day decay + 2 seeded older-half picks; deviation: WindowItem snapshot type, not a memory.Store selector — store exposes no enumeration/salience (§2.8 forbids world-side salience); 33 tests -race green; orchestrator re-ran vet+test green uncached), 4 done (b179249/86dd82c/631ab68, sonnet verified, ~200k; evening_test.go — three postings over a real net.Pipe wire against FakeVendor, one intent each, no re-posting, persona-grounded decline naming a prior commitment, -race -count=20; overview + promptworld-lineage NEEDS-REVIEW amended + re-pinned, CAPSULES regenerated; 13 ACs ticked with citing proofs). Orchestrator: origin/main moved post-branch (253b337) — merged in (1447a91), gates + probe re-run: green except villager-brain-api STALE, which is pre-existing main-side debt (pin 2b19674 missed TASK-0014's 22e93e1/9b1ca21) already fixed honestly on the TASK-0017 branch (6e03135, rides PR #21) — not re-amended here to avoid conflicting fixes of one note. PR #22 MERGED 27be6ae (operator-authorized sweep merge; reconciled with post-#21 main first — overview union-merged, probe green); card synced Done (b00e5fe); worktree+branch removed after merge verified. ~692k subagent tokens across 4 sonnet dispatches. |
| 2026-08-28 | TASK-0017 | — | — | — | in flight (lane 4, parallel with TASK-0016): claimed (c0f48c6), spec cycle (a241cb0), phase ACs seeded + tier note (ddaccdb). phases: 1 done (169ab19, sonnet VERIFIED serving claude-sonnet-5, ~206k; mind/converse — two-Speaker exchange on E4 registry config, stable-prefix byte-identity test, closing-marker termination with never-firing safety bound, live fakevendor e2e over net.Pipe with SSE mock + injected first-token delay <3s; deviations recorded: speak text rides target {\"type\":\"none\",\"text\":...} — §5.2 leaves no content field; race in own test found+fixed under -race; orchestrator re-ran vet+test green), 2 done (b18adf7, sonnet verified, ~139k; Slot/Pool pregen keyed (pairID,day) — one-shot Take makes fill/convergence race safe both orders incl. 25 randomized -race trials, late fill discarded, live fallback when unfilled incl. failed-fill side effect; Exchange turn-0 slot consult with counting-server proofs of no-new-call; OpeningWait defaults 0 non-blocking per TASK-0014's short-lead finding; orchestrator re-ran vet+test green uncached), 3 done (86f1484, sonnet verified, ~151k; AmbientPool batched E5 Haiku, <200ms measured serve, no intra-cycle repeat, day-rollover; IsTargeted/Escalate live-call bypass; model-free Stall structurally unable to call), 4 done (e86913c/72ffa64/79258fc, sonnet verified, ~154k; stall-only-after-exhaustion proof, politeness source-scan over both classes + verbatim-resentment pass-through, overview NEEDS-REVIEW amended + re-pinned, 12 ACs ticked with honest-limit notes on #3/#4). Orchestrator: base unmoved, gates re-run green uncached, freshness probe caught villager-brain-api staled on MAIN pre-branch (pin 2b19674 missed TASK-0014's 22e93e1/9b1ca21) — NEEDS-REVIEW classified, chunk-ticket trap folded in, honest re-pin (6e03135). PR #21 MERGED 287cf03 by operator authorization (merge commit; self-merge had been classifier-denied; operator then authorized sweep merges for the session); card synced Done (b00e5fe); worktree+branch removed after merge verified. ~650k subagent tokens across 4 sonnet dispatches. |
