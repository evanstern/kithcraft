...
- [ ] promptworld I mind-side machinery classified (INDEX-first from its docs/wiki): portable-behind-the-seam vs entangled-with-dead-engine, per doctrine item (event-sourced memory, reflex/planner, salience/consolidation, epistemic hygiene, persona firewall)
- [ ] Seam-contract demands on any daemon extracted from docs/design/body-protocol-v0.md (surfaces to implement, fake-vendor testability)
- [ ] Candidate languages enumerated with evidence (SDK maturity, async fit, maintainability), each claim referenced
- [ ] Evidence file specs/004-mind-daemon-routing/research/daemon-assessment.md committed
