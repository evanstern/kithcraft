...
- [x] #4 Operator has ratified the decisions
...
Both decisions ratified (operator merged PR #9, merge 43e528a, 2026-08-21; decision-0003 proposed→accepted). Deliverables: decision-0003 (mind daemon in Go, rebuilt behind the seam — four promptworld I assets reused as source material, keep-Go collapsed by measurement, JVM rejected on SI-1 risk, Agent SDK rejected on fit); docs/design/llm-routing-and-budget.md (3-tier routing, demo evening ≈ $5.17 / $3.99 cached, sensitivity to $16-17); daemon-assessment evidence; promptworld-lineage re-verified + re-pinned, CAPSULES regenerated. Q-1 transport narrowed one-way (in-process foreclosed) — flagged for the seam owner, not decided. All 12 spec boxes ticked; all three phases served by verified cc/claude-opus-5[1m].
